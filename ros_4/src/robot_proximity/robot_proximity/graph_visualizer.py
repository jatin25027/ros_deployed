...
class GraphVisualizer(Node):
    def __init__(self):
        ...
        if spawn_interval > 0:
            ...
            self.get_logger().info(f"(OK) GraphVisualizer Node fully active. DYNAMIC OBSTACLES: ON ({spawn_interval}s interval)")
        else:
            self.get_logger().info("(OK) GraphVisualizer Node fully active. DYNAMIC OBSTACLES: OFF")
